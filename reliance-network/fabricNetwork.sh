...
function printHelp() {
  ...
  echo "    -c <channel name> - channel name to use (defaults to \"channelfiveorgs\")"
  ...
  echo
  echo "	network.sh generate -c channelfiveorgs"
  echo "	network.sh up -c channelfiveorgs -s couchdb"
  echo "        network.sh up -c channelfiveorgs -s couchdb -i 1.4.0"
  echo "	network.sh up -l node"
  echo "	network.sh down -c channelfiveorgs"
  echo
...
function checkPrereqs() {
  ...
  for UNSUPPORTED_VERSION in $BLACKLISTED_VERSIONS; do
    ...
    if [ $? -eq 0 ]; then
      echo "ERROR! Local Fabric binary version of $LOCAL_VERSION does not match this newer version of channelfiveorgs and is unsupported. Either move to a later version of Fabric or checkout an earlier version of channelfiveorgs."
      exit 1
    fi

    echo "$DOCKER_IMAGE_VERSION" | grep -q "$UNSUPPORTED_VERSION"
    if [ $? -eq 0 ]; then
      echo "ERROR! Fabric Docker image version of $DOCKER_IMAGE_VERSION does not match this newer version of channelfiveorgs and is unsupported. Either move to a later version of Fabric or checkout an earlier version of channelfiveorgs."
      exit 1
    fi
  done
}
...
function setupComposer() {
  ...
  composer card create -p ./"$CONNECTION_PROFILE" -u ansar -n reliance-network -c ansar/admin-pub.pem -k ansar/admin-priv.pem

  # Check if a card with the same name has previously been imported? If yes, remove it before importing a new one.
  if composer card list -c ansar@reliance-network >/dev/null; then
    composer card delete -c ansar@reliance-network
  fi

  # Import the business network card into wallet for Infrastructure admin user [Ansar]
  composer card import -f ansar@reliance-network.card
  ...
  composer card import -f importer.card

  # Test the business network access of Infrastructure Pay user
  composer network ping -c importer@reliance-network
}
...
function replacePrivateKey() {
  ...
  cd crypto-config/peerOrganizations/communications.reliance-network.com/ca/ || exit
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
  sed $OPTS "s/COMMUNICATIONS_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose.yml
...
# (``peer0.infrastructure.reliance-network.com`` & ``peer0.power.reliance-network.com``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  set -x
  if [ "$CONSENSUS_TYPE" == "solo" ]; then
    configtxgen -profile OrdererGenesis -channelID reliance-sys-channel -outputBlock ./channel-artifacts/genesis.block
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile ChannelFiveOrgs -outputCreateChannelTx ./channel-artifacts/channel.tx -channelID "$CHANNEL_NAME"
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile ChannelFiveOrgs -outputAnchorPeersUpdate ./channel-artifacts/InfrastructureMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg InfrastructureMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for InfrastructureMSP..."
    exit 1
  fi
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile ChannelFiveOrgs -outputAnchorPeersUpdate \
    ./channel-artifacts/PowerMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg PowerMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for PowerMSP..."
    exit 1
  fi
  echo
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for CommunicationsMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile ChannelFiveOrgs -outputAnchorPeersUpdate \
    ./channel-artifacts/CommunicationsMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg CommunicationsMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for CommunicationsMSP..."
    exit 1
  fi
  echo
  ...
  set -x
  configtxgen -profile ChannelFiveOrgs -outputAnchorPeersUpdate \
    ./channel-artifacts/EntertainmentMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg EntertainmentMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for EntertainmentMSP..."
    exit 1
  fi
  echo
  ...
  set -x
  configtxgen -profile ChannelFiveOrgs -outputAnchorPeersUpdate \
    ./channel-artifacts/CapitalMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg CapitalMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for CapitalMSP..."
    exit 1
  fi
  echo
...
CLI_DELAY=5
# channel name defaults to "channelfiveorgs"
...
MODE=$1
shift
# Determine whether starting, stopping, restarting, generating or relianceing
if [ "$MODE" == "up" ]; then
  EXPMODE="Starting"
elif [ "$MODE" == "down" ]; then
  EXPMODE="Stopping"
