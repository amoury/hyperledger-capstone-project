...
PEER0_COMMUNICATIONS_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/communications.reliance-network.com/peers/peer0.communications.reliance-network.com/tls/ca.crt
...
setGlobals() {
  ...
  if [ $ORG == 'infrastructure' ]; then
    ...
    CORE_PEER_TLS_ROOTCERT_FILE=$PEER0_COMMUNICATIONS_CA
...
instantiateChaincode() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer chaincode instantiate -o orderer.reliance-network.com:7050 -C $CHANNEL_NAME -n edtech -l ${LANGUAGE} -v ${VERSION} -c '{"Args":["org.reliance-network.edtech:instantiate"]}' -P "OR ('InfrastructureMSP.member','PowerMSP.member','CommunicationsMSP.member','EntertainmentMSP.member','CapitalMSP.member')" >&log.txt
    res=$?
    set +x
  else
    set -x
    peer chaincode instantiate -o orderer.reliance-network.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n edtech -l ${LANGUAGE} -v ${VERSION} -c '{"Args":["org.reliance-network.edtech:instantiate"]}' -P "OR ('InfrastructureMSP.member','PowerMSP.member','CommunicationsMSP.member','EntertainmentMSP.member','CapitalMSP.member')" >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
...
relianceeChaincode() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer chaincode reliancee -o orderer.reliance-network.com:7050 -C $CHANNEL_NAME -n edtech -l ${LANGUAGE} -v ${VERSION} -p ${CC_SRC_PATH} -c '{"Args":["org.reliance-network.edtech:instantiate"]}' -P "OR ('InfrastructureMSP.member','PowerMSP.member','CommunicationsMSP.member','EntertainmentMSP.member','CapitalMSP.member')" >&log.txt
    res=$?
    set +x
  else
    set -x
    peer chaincode reliancee -o orderer.reliance-network.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n edtech -l ${LANGUAGE} -v ${VERSION} -p ${CC_SRC_PATH} -c '{"Args":["org.reliance-network.edtech:instantiate"]}' -P "OR ('InfrastructureMSP.member','PowerMSP.member','CommunicationsMSP.member','EntertainmentMSP.member','CapitalMSP.member')" >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
  verifyResult $res "Chaincode reliancee on peer${PEER}.${ORG} has failed"
  echo "===================== Chaincode is relianceed on peer${PEER}.${ORG} on channel '$CHANNEL_NAME' ===================== "
  echo
}
...
chaincodeInvoke() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer chaincode invoke -o orderer.reliance-network.com:7050 -C $CHANNEL_NAME -n edtech $PEER_CONN_PARMS -c '{"Args":["org.reliance-network.edtech:create","0001","Medicine","2","Capital-Comm"]}' >&log.txt
    res=$?
    set +x
  else
    set -x
    peer chaincode invoke -o orderer.reliance-network.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n edtech $PEER_CONN_PARMS -c '{"Args":["org.reliance-network.edtech:create","0001","Medicine","2","Capital-Comm"]}' >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
